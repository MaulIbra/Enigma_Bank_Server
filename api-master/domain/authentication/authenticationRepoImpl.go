...
func (a AuthenticationRepo) AddUserProfile(profile *models.Profile) error {
	...
	if _, err := stmt.Exec(id, profile.User.Username, profile.User.Password,profile.User.Balance); err != nil {
		tx.Rollback()
		log.Print(err)
		return err
	}
...
func (a AuthenticationRepo) ReadUserByEmail(username string) (*models.User, error) {
	...
	errQuery := stmt.QueryRow(username).Scan(&u.UserID, &u.Username, &u.Password,&u.Balance)
